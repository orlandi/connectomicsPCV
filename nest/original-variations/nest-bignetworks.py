...
inputList = ['test', 'valid']

if len(cmd_arguments) != 2:
  print "usage: ./CHA-adaptive-bursts.py list_iterator (starts at 1)"
  sys.exit(0)

YAMLinputfilename = "../../networks/original/" + inputList[int(cmd_arguments[1])-1] + "-withShufflingData.yaml"
spiketimefilename = "../../data/original-variations/spikes/" + inputList[int(cmd_arguments[1])-1] + "-times.txt"
spikeindexfilename = "../../data/original-variations/spikes/" + inputList[int(cmd_arguments[1])-1] + "-idx.txt"
...
for iSubNet in SUBNET_INDICES:
  ...
  while abs(burst_rate-TARGET_BURST_RATE)>TARGET_BURST_RATE_ACCURACY_GOAL:
    ...
    [size,cons,neuronsE,espikes,noise,GIDoffset] = nest_meta.go_create_network(yamlobj, weight, WEIGHT_NOISE, NOISE_RATE, False, 1.0, iSubNet, True)
