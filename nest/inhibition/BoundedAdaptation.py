...
class BoundedAdaptationRunner:
  ...
  def next(self, last_value_of_y):
    ...
    if self.__iteration_count() < 2:
      ...
      if last_value_of_y > self.target_value_of_y:
        next_value_of_x = self.last_x_values[-1] * 0.9
      else:
        next_value_of_x = self.last_x_values[-1] * 1.1
      ...
      # Check for too big a jump
      if next_value_of_x/self.last_x_values[-1] > 2.0:
        print "DEBUG: Extrapolated value of {w} is too big, bounding it...".format(w=next_value_of_x)
        next_value_of_x = self.last_x_values[-1] * 2.0
      elif next_value_of_x/self.last_x_values[-1] < 0.5:
        print "DEBUG: Extrapolated value of {w} is too big, bounding it...".format(w=next_value_of_x)
        next_value_of_x = self.last_x_values[-1] * 0.5
